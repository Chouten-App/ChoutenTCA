...
@dynamicMemberLookup
 class JSValueEncoder {
   ...
   typealias NonConformingFloatEncodingStrategy = JSONEncoder.NonConformingFloatEncodingStrategy
  ...
  private var userInfo: [CodingUserInfoKey: Any] = [:]
   ...
   subscript<V>(dynamicMember keyPath: WritableKeyPath<Options, V>) -> V {
    get { self.options[keyPath: keyPath] }
    set { self.options[keyPath: keyPath] = newValue }
  }

   func encode(_ value: some Encodable, into context: JSContext) throws -> JSValue {
    let encoder = Encoder(
      context: context,
      userInfo: userInfo,
      options: options
