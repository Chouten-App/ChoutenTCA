...
extension RepoFeature.View {
  // swittlint:disable identifier_name
  @MainActor
        ...
        VStack(alignment: .leading) {
          ...
          ForEach(getFilteredModules().enumerated().map { $0 }, id: \.element) { index, module in
            ModuleButton(
              ...
              index: index,
              state: index == 0 ? ModuleVersionStatus.updateAvailable : ModuleVersionStatus.upToDate
            )
