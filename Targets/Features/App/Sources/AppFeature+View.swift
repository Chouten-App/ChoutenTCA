                                    ...
                                    ScrollView(.horizontal) {
                                        HStack {
                                            ForEach(0..<viewStore.mediaItems.count, id: \.self) { index in
                                                ...
                                                ContinueCard(item: item, module: module, show: $showContextMenu, press: $press, hoveredIndex: $hoveredIndex, index: index, changeMediaData: $changeMediaData, showAlert: $showAlert, viewStore: viewStore)
...
struct ContinueCard: View {
    ...
    var formattedValue: String {
        if item.number.truncatingRemainder(dividingBy: 1) == 0 {
            return String(format: "%.0f", item.number)
        } else {
    ...
    init(item: Media,
        ...
        if let documentsDirectory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first, let module {
            let fileURL = documentsDirectory
                ...
                .appendingPathComponent(item.mediaUrl
